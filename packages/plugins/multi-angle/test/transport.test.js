...
describe("multi-angle v8 transport", () => {
  test("uses only host/8 and declared Catalog API ids", async () => {
    const [application, sdkClient, manifest, metadata, workspace, publication] =
      await Promise.all([
    ...
    expect(
      publication.requirements
        .filter((requirement) =>
          requirement.affected.some((item) => item.id === "multi-angle"),
        )
    ...
    expect(application).toContain(
      'GENERATE_MESSAGE = "renderer.multi-angle.generate"',
    )
    expect(application).toContain(
      'REFRESH_MESSAGE = "renderer.multi-angle.refresh"',
    )
  ...
  test("closes each acquired bearer session exactly once on success, failure, and cancellation", async () => {
    ...
    for (const outcome of ["success", "failure", "cancel"]) {
      ...
      if (outcome === "success")
        await expect(result).resolves.toBe("preview-ready")
      else
        await expect(result).rejects.toThrow(
          outcome === "failure" ? "decode failed" : "stale input",
        )
