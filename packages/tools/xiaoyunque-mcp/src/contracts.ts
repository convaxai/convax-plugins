...
export const maximumGenerationPromptBytes = 20_000;

export class XiaoYunquePromptTooLongError extends Error {
  override name = "XiaoYunquePromptTooLongError";

  constructor() {
    super("XiaoYunque generation prompt is too long");
  }
}
...
export function parseGenerationPrompt(value: unknown) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value !== value.trim() ||
    ...
    throw new Error("generation prompt must be non-empty trimmed text");
  }
  if (Buffer.byteLength(value, "utf8") > maximumGenerationPromptBytes) {
    throw new XiaoYunquePromptTooLongError();
  }
  return value;
}
...
export function parseGenerationCall(
  value: unknown,
  expectedOutput: GenerationOutput,
): GenerationCall {
  ...
  if (unknown)
    throw new Error(`generation call contains unsupported field: ${unknown}`);
  if (input.schema !== generationCallSchema)
    throw new Error("generation call schema is not supported");
  if (input.output !== expectedOutput)
    throw new Error("generation call output does not match the selected tool");
  if (!Array.isArray(input.references) || input.references.length > 16) {
    throw new Error(
      "generation references must be an array with at most 16 entries",
    );
  }
  const references = input.references.map(
    (value, index): GenerationReference => {
      const reference = asRecord(value, `generation reference ${index}`);
      const role = reference.role;
      if (
        typeof role !== "string" ||
        !referenceRoles.has(role as GenerationReferenceRole)
      ) {
        throw new Error(
          `generation reference ${index} has an unsupported role`,
        );
      }
      const nodeId = requireTrimmedString(
        reference.node_id,
        `generation reference ${index} node_id`,
        256,
      );
      if (reference.kind === "text") {
        const text = requireTrimmedText(
          reference.text,
          `generation reference ${index} text`,
          200_000,
        );
        if (role !== "text")
          throw new Error("text generation references must use the text role");
        return { kind: "text", node_id: nodeId, role: "text", text };
      }
      if (reference.kind !== "file")
        throw new Error(`generation reference ${index} kind is not supported`);
      if (role === "text")
        throw new Error("file generation references cannot use the text role");
      return {
        kind: "file",
        mime_type: requireTrimmedString(
          reference.mime_type,
          `generation reference ${index} mime_type`,
          256,
        ),
        name: requireTrimmedString(
          reference.name,
          `generation reference ${index} name`,
          512,
        ),
        node_id: nodeId,
        path: requireTrimmedString(
          reference.path,
          `generation reference ${index} path`,
          4_096,
        ),
  ...
  return {
    operation_id: requireTrimmedString(
      input.operation_id,
      "generation operation_id",
      256,
    ),
    output: expectedOutput,
    output_directory: requireTrimmedString(
      input.output_directory,
      "generation output_directory",
      4_096,
    ),
    prompt: parseGenerationPrompt(input.prompt),
    references,
