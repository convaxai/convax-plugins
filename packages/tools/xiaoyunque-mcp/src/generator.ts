...
type GenerationInputErrorReason =
  | "invalid-frame-combination"
  | "last-frame-without-first";
...
export class XiaoYunqueObservationRejectedError extends Error {
  ...
  constructor(
    upstreamDiagnosticCode: XiaoYunqueRequestRejectionDiagnosticCode,
  ) {
...
export type XiaoYunqueTerminalGenerationReason =
  | "failed"
  | "cancelled"
  | "additional-input"
  | "human-input";

export class XiaoYunqueTerminalGenerationError extends Error {
  override name = "XiaoYunqueTerminalGenerationError";
  readonly reason: XiaoYunqueTerminalGenerationReason;

  constructor(reason: XiaoYunqueTerminalGenerationReason) {
    super(`XiaoYunque generation entered terminal state: ${reason}`);
    this.reason = reason;
  }
}
...
function fileReferences(call: GenerationCall) {
  return call.references.filter(
    (reference): reference is FileGenerationReference =>
      reference.kind === "file",
  );
}
...
function requireImageReferences(call: GenerationCall) {
  ...
  if (call.references.some((reference) => reference.kind === "text")) {
    throw new Error(
      "The XiaoYunque image tool does not accept Canvas text references",
    );
  }
...
function videoReferences(call: GenerationCall) {
  if (call.references.some((reference) => reference.kind === "text")) {
    throw new Error(
      "The XiaoYunque video tool does not accept Canvas text references",
    );
  }
  const files = fileReferences(call);
  const firstFrames = files.filter(
    (reference) => reference.role === "first_frame",
  );
  const lastFrames = files.filter(
    (reference) => reference.role === "last_frame",
  );
  const ordinaryImages = files.filter(
  ...
  const usesFirstAndLastFrames =
    firstFrames.length === 1 && lastFrames.length === 1;
  ...
  if (knownCount !== files.length)
    throw new Error(
      "The XiaoYunque video tool received an unsupported reference role",
    );
  if (images.length > 9)
    throw new Error("XiaoYunque accepts at most nine reference images");
  if (videos.length > 3)
  ...
  return {
    audios,
    generateType: usesFirstAndLastFrames ? 1 : undefined,
    images,
    videos,
  };
}
...
export class GenerationEngine {
  ...
  async generate(
    ...
    if (!modelSupportsOutput(model, call.output)) {
      throw new Error(
        "The selected XiaoYunque model does not match the generation output",
      );
    }
    const outputDirectory = await this.#validateOutputDirectory(
      call.output_directory,
    );
    const fingerprint = await fingerprintGenerationCall(call, model, signal);
    const task = await this.#operationStore.withOperationLock(
      call.operation_id,
      signal,
      async () => {
        ...
        if (cached) {
          if (
            ...
            cached.output !== call.output
          ) {
            throw new Error(
              "Generation operation id was reused with a different request",
            );
          }
          if (cached.status === "submitting") {
            throw new Error(
              "A prior XiaoYunque submission may have been accepted; refusing to submit it again",
            );
          }
    ...
    if (urls.length === 0) {
      throw new Error(
        result.error ||
          `XiaoYunque completed without a ${call.output} artifact`,
      );
    }
  ...
  async #submit(
    ...
    if (call.output === "image") {
      ...
      for (const reference of references)
        assets.push(await this.#api.upload(reference, session, signal));
      if (signal.aborted) throw abortError();
    ...
    const uploadAll = async (values: FileGenerationReference[]) => {
      ...
      for (const reference of values)
        result.push(await this.#api.upload(reference, session, signal));
      return result;
    };
    ...
    return this.#api.submitVideo(
      {
        audioAssets,
        beforeSubmit,
        ...(references.generateType === undefined
          ? {}
          : { generateType: references.generateType }),
        imageAssets,
  ...
  async #poll(task: RemoteTask, output: GenerationOutput, signal: AbortSignal) {
    ...
    while (true) {
      ...
      if (result.state === 4 || result.state === 5) {
        ...
        throw new XiaoYunqueTerminalGenerationError(
          result.state === 4 ? "failed" : "cancelled",
        );
      }
      if (result.state === 6) {
        throw new XiaoYunqueTerminalGenerationError("additional-input");
      }
      if (result.state === 9) {
        throw new XiaoYunqueTerminalGenerationError("human-input");
      }
      await abortableDelay(this.#pollIntervalMs, signal);
    }
  }
  ...
  async #validateOutputDirectory(directory: string) {
    if (!path.isAbsolute(directory))
      throw new Error("Generation output_directory must be absolute");
    const info = await lstat(directory);
    if (!info.isDirectory() || info.isSymbolicLink())
      throw new Error("Generation output_directory is not a real directory");
    return directory;
  }
  ...
  async #download(
    ...
    const temporaryPath = path.join(
      outputDirectory,
      `.xiaoyunque-${crypto.randomUUID()}.download`,
    );
    ...
    try {
      ...
      for await (const value of response.stream) {
        ...
        for (const byte of value.subarray(0, Math.max(0, 512 - prefix.length)))
          prefix.push(byte);
        let offset = 0;
        while (offset < value.byteLength) {
          ...
          if (bytesWritten <= 0)
            throw new Error("Unable to stage XiaoYunque artifact");
          offset += bytesWritten;
        }
      }
