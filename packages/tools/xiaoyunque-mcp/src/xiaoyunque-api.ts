...
function looksLikeAuthenticationFailure(
...
function requireSuccess(value: unknown, status: number, label: string) {
  ...
  if (looksLikeAuthenticationFailure(envelope, status)) {
    throw new XiaoYunqueAuthenticationError(
      "XiaoYunque authorization is no longer valid",
    );
  }
...
async function boundedJson(response: Response, label: string) {
  ...
  if (declaredLength !== null) {
    ...
    if (
      ...
      parsed < 0 ||
      parsed > maximumJsonResponseBytes
    ) {
      await response.body?.cancel().catch(() => undefined);
...
function uploadAssetType(reference: FileGenerationReference): UploadAssetType {
  if (
    ...
    return 1;
  if (reference.role === "audio" || reference.mime_type.startsWith("audio/"))
    return 4;
  if (
...
function registeredAssetTypeForUpload(
  assetType: UploadAssetType,
): RegisteredAssetType | undefined {
  switch (assetType) {
    case 1: // video
      return 2;
    case 2: // image
...
function uploadedAsset(
  ...
  if (!data)
    throw new Error("XiaoYunque reference upload returned an invalid asset");
  const assetId = boundedString(
    data.asset_id,
    "XiaoYunque uploaded asset id",
    1_024,
  );
  const url = strictUrl(
    data.download_url,
    "XiaoYunque uploaded asset URL",
    allowLoopbackHttp,
  );
  const width = optionalNonNegativeInteger(
    data.width,
    "XiaoYunque uploaded asset width",
  );
  const height = optionalNonNegativeInteger(
    data.height,
    "XiaoYunque uploaded asset height",
  );
  const durationMilliseconds = optionalNonNegativeInteger(
    data.duration_ms,
    "XiaoYunque uploaded asset duration",
  );
  const size = optionalNonNegativeInteger(
    data.size,
    "XiaoYunque uploaded asset size",
  );
  const format = optionalBoundedString(
    data.format,
    "XiaoYunque uploaded asset format",
    128,
  );
  const md5 = optionalBoundedString(
    data.md5,
    "XiaoYunque uploaded asset md5",
    256,
  );
  const mime = optionalBoundedString(
    data.mime,
    "XiaoYunque uploaded asset MIME type",
    256,
  );
  ...
  return {
    ...
    name: boundedString(
      reference.name,
      "XiaoYunque uploaded asset name",
      1_024,
    ),
...
function requestAsset(asset: UploadedAsset) {
  const metadata = {
    ...
    ...(asset.metadata.format === undefined
      ? {}
      : { format: asset.metadata.format }),
    ...(asset.metadata.md5 === undefined ? {} : { md5: asset.metadata.md5 }),
    ...(asset.metadata.frameCount === undefined
      ? {}
      : { frame_cnt: asset.metadata.frameCount }),
    ...
    ...(asset.metadata.ratio === undefined
      ? {}
  ...
  return {
    ...
    ...(asset.pippitAssetId === undefined
      ? {}
      : { pippit_asset_id: asset.pippitAssetId }),
    metadata,
    name: asset.name,
    url: asset.url,
  };
...
function runContext(
  ...
  const osName =
    options.includeOperatingSystem === false
      ? undefined
...
function strictRunState(value: unknown) {
  if (typeof value === "string" && /^(?:0|[1-9])$/u.test(value))
    return Number(value);
  if (
    typeof value === "number" &&
...
function failureReason(value: unknown) {
  ...
  if (!reason)
    throw new Error("XiaoYunque generation failure reason is invalid");
  for (const candidate of [
    reason.message,
    reason.fallback_message,
    reason.detail,
    reason.error_cta_message,
  ]) {
    if (candidate === undefined || candidate === null || candidate === "")
      continue;
    return boundedString(
      candidate,
      "XiaoYunque generation failure reason",
      2_000,
    );
  }
  return "";
...
function artifactUrl(value: unknown) {
  ...
  if (typeof sceneUrls?.download === "string" && sceneUrls.download.trim())
    return sceneUrls.download;
  return (
    media?.url ??
    media?.previewUrl ??
    media?.preview_url ??
...
function artifactUrls(
  ...
  for (const entryValue of entriesValue) {
    ...
    if (
      ...
      container.content.length > maximumContentCount
    ) {
    ...
    for (const partValue of container.content) {
      ...
      if (
        part.sub_type !== "biz/x_data_image" &&
      ...
      const media =
        part.sub_type === "biz/x_data_image" ? data?.image : data?.video;
      ...
      const target =
        part.sub_type === "biz/x_data_image" ? imageUrls : videoUrls;
...
export class XiaoYunqueApi {
  ...
  constructor(
    ...
    this.#allowLoopbackTest =
      this.#baseUrl.protocol === "http:" &&
      this.#baseUrl.hostname === "127.0.0.1";
    if (
      this.#baseUrl.origin !== xiaoYunqueCookieOrigin &&
      !this.#allowLoopbackTest
    ) {
    ...
    this.#queryRequestTimeoutMs =
      options.queryRequestTimeoutMs ?? defaultQueryRequestTimeoutMs;
    if (
  ...
  async upload(
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new Error("Generation reference is not a regular file");
    ...
    const response = await this.#request(uploadPath, session, signal, {
      body,
      method: "POST",
    });
    ...
    if (registeredAssetType === undefined || asset.pippitAssetId !== undefined)
      return asset;
  ...
  async #registerUploadedAsset(
    ...
    try {
      ...
      const payload = requireAssetRegistrationSuccess(
        response.value,
        response.status,
      );
  ...
  async submitImage(
    ...
    const prompt = parseGenerationPrompt(options.prompt.trim());
    if (options.assets.length > 9)
      throw new Error("XiaoYunque accepts at most nine reference images");
  ...
  async submitVideo(
    ...
    if (options.imageAssets.length > maximumImages) {
      throw new Error(
        `XiaoYunque ${options.model} accepts at most ${maximumImages} reference image${maximumImages === 1 ? "" : "s"}`,
      );
    }
    if (options.videoAssets.length > 3)
      throw new Error("XiaoYunque accepts at most three reference videos");
    if (options.audioAssets.length > 3)
      throw new Error("XiaoYunque accepts at most three reference audio files");
    if (
      !videoModelSupportsVideoReferences(options.model) &&
      options.videoAssets.length > 0
    ) {
      throw new Error(
        `XiaoYunque ${options.model} does not accept reference videos`,
      );
    }
    if (
      !videoModelSupportsAudioReferences(options.model) &&
      options.audioAssets.length > 0
    ) {
      throw new Error(
        `XiaoYunque ${options.model} does not accept reference audio`,
      );
    }
    if (
      !videoModelSupportsFirstAndLastFrames(options.model) &&
      options.generateType !== undefined
    ) {
      throw new Error(
        `XiaoYunque ${options.model} does not accept first and last frames`,
      );
    }
  ...
  async #withQueryRequestTimeout<T>(
    ...
    const interruption = new Promise<never>((_resolve, reject) => {
      ...
      timer = setTimeout(() => {
        controller.abort();
        reject(
          new XiaoYunqueQueryTimeoutError(
            "XiaoYunque generation status request timed out",
          ),
        );
      }, this.#queryRequestTimeoutMs);
    });
  ...
  async #submit(
    ...
    const payload = requireSuccess(
      response.value,
      response.status,
      `XiaoYunque ${options.editType} generation`,
    );
    if (!record(payload.data))
      throw new Error(
        "XiaoYunque generation submission returned an invalid response",
      );
  }
  ...
  async #userInfo(
    session: StoredWebSession,
    signal: AbortSignal,
  ): Promise<XiaoYunqueUserInfo> {
    const identityResponse = await this.#jsonRequest(
      odinUserInfoPath,
      {},
      session,
      signal,
    ...
    if (!identity)
      throw new Error(
        "XiaoYunque identity lookup returned an invalid response",
      );
    const consumerUid = boundedString(
      identity.user_id,
      "XiaoYunque consumer uid",
      1_024,
    );

    const workspaceResponse = await this.#jsonRequest(
      userWorkspacePath,
      {
        uid: consumerUid,
      },
    ...
    if (!workspace)
      throw new Error(
        "XiaoYunque workspace lookup returned an invalid response",
      );
    return {
      consumer_uid: consumerUid,
      ...(workspace.space_id === undefined ||
      workspace.space_id === null ||
      workspace.space_id === ""
        ? {}
        : {
            space_id: boundedString(
  ...
  async #queryRun(
    ...
    const payload = requireSuccess(
      response.value,
      response.status,
      "XiaoYunque generation status",
    );
    const data = record(payload.data);
    const thread = record(data?.thread);
    if (
      !thread ||
      !Array.isArray(thread.run_list) ||
      thread.run_list.length > 256
    ) {
    ...
    const run = thread.run_list
      .map(record)
      .find((candidate) => candidate?.run_id === task.runId);
    if (!run)
      return {
        run_id: task.runId,
        thread_id: task.threadId,
  ...
  async #request(
    ...
    const cookieUrl = this.#allowLoopbackTest
      ? new URL(path, xiaoYunqueCookieOrigin)
      : url;
