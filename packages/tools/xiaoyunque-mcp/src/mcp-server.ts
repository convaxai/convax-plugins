...
interface PluginServiceExecutor {
  authorize(
    signal?: AbortSignal,
  ): Promise<PluginServiceBrowserAuthorizationRequest>;
...
export const generationMcpTools = generationTools.map((tool) => ({
  ...
  inputSchema: {
    ...
    properties: {
      ...
      output: { const: tool.output, type: "string" },
    },
    required: [
      "schema",
      "operation_id",
...
export const serviceMcpTools = [
  {
    description:
      "Report bounded XiaoYunque account, credit, usage, and local authorization status.",
    inputSchema: {
      additionalProperties: false,
      properties: {},
      type: "object",
  ...
  {
    description:
      "Request a host-managed XiaoYunque browser login and whitelisted Cookie capture.",
    inputSchema: {
      additionalProperties: false,
      properties: {},
      type: "object",
  ...
  {
    description:
      "Request host-managed XiaoYunque reauthorization without replacing the current session yet.",
    inputSchema: {
      additionalProperties: false,
      properties: {},
      type: "object",
  ...
  {
    description:
      "Cancel the currently active XiaoYunque browser authorization, if any.",
    inputSchema: {
      additionalProperties: false,
      properties: {},
      type: "object",
  ...
  {
    description:
      "Complete the active host-managed browser authorization with its bounded Cookie envelope.",
    inputSchema: {
      additionalProperties: false,
      properties: {
        authorization_id: {
        ...
        schema: {
          const: "convax.plugin-service-browser-authorization-completion/1",
          type: "string",
        },
      },
...
const serviceToolNames: ReadonlySet<string> = new Set(
  serviceMcpTools.map((tool) => tool.name),
);
...
export function publicGenerationErrorMessage(error: unknown) {
  if (error instanceof XiaoYunqueCredentialConfigurationError)
    return error.publicMessage;
  if (error instanceof XiaoYunqueGenerationInputError)
    return error.publicMessage;
  if (error instanceof XiaoYunquePromptTooLongError) {
    return "The XiaoYunque prompt is too long. Shorten the prompt or remove some @ text context and try again.";
  }
  ...
  if (error instanceof XiaoYunqueTerminalGenerationError) {
    if (error.reason === "failed") {
      return "XiaoYunque accepted the generation, but the service reported that it failed. Check XiaoYunque before starting another paid generation.";
    }
    if (error.reason === "cancelled") {
      return "XiaoYunque accepted the generation, but the service reported that it was cancelled. Check XiaoYunque before starting another paid generation.";
    }
    if (error.reason === "additional-input") {
      return "XiaoYunque accepted the generation, but it requires additional input in XiaoYunque. Complete it there before starting another paid generation.";
    }
    return "XiaoYunque accepted the generation, but it was interrupted for human input. Check XiaoYunque before starting another paid generation.";
  }
...
export type SafeGenerationDiagnosticCode =
  (typeof safeGenerationDiagnosticCodes)[number];

export function safeGenerationDiagnosticCode(
  error: unknown,
): SafeGenerationDiagnosticCode {
  if (error instanceof XiaoYunqueCredentialConfigurationError)
    return "local-setup-required";
  if (error instanceof XiaoYunqueGenerationInputError)
    return "local-input-rejected";
  if (error instanceof XiaoYunquePromptTooLongError) return "prompt-too-long";
  if (error instanceof XiaoYunqueAuthenticationError) return "sign-in-expired";
  if (error instanceof XiaoYunqueObservationRejectedError)
    return "status-check-rejected";
  if (error instanceof XiaoYunqueQueryTimeoutError)
    return "status-check-timeout";
  if (error instanceof XiaoYunqueUnsupportedImageModelError)
    return "unsupported-image-model";
  if (error instanceof XiaoYunqueTerminalGenerationError) {
    return error.reason === "failed"
      ? "generation-failed"
      : error.reason === "cancelled"
        ? "generation-cancelled"
        : error.reason === "additional-input"
          ? "generation-input-required"
          : "generation-interrupted";
  }
  if (error instanceof XiaoYunqueReferenceAssetRegistrationError) {
  ...
  async #handle(value: unknown) {
    ...
    if (request.method === "initialize") {
      ...
      this.#sendResult(request.id, {
        capabilities: { tools: {} },
        protocolVersion,
        serverInfo: { name: "convax-xiaoyunque-mcp", version: "0.3.6" },
      });
      return;
    }
  ...
  async #callTool(request: JsonRpcRequest & { id: number | string }) {
    ...
    try {
      ...
      if (
        ...
        serviceToolNames.has(params.name)
      ) {
        if (controller.signal.aborted)
          throw new DOMException("Service action was cancelled", "AbortError");
        ...
        if (params.name === "service.authorization.complete") {
          ...
          if (Object.keys(input).length !== 0) {
            this.#sendError(
              request.id,
              -32602,
              "This service tool does not accept arguments",
            );
            return;
          }
          structuredContent =
            params.name === "service.status"
      ...
      const artifacts = await this.#engine.generate(
        call,
        tool.model,
        controller.signal,
      );
      const result: ToolResult = {
        content: [
          {
            type: "text",
            text: `Generated ${artifacts.length} ${output} artifact${artifacts.length === 1 ? "" : "s"}.`,
          },
        ],
      ...
      const serviceCall = (() => {
        try {
          const params = asRecord(request.params, "tools/call params");
          return (
            typeof params.name === "string" && serviceToolNames.has(params.name)
          );
        } catch {
          return false;
        }
        ...
        content: [
          {
            ...
            text: serviceCall
              ? "XiaoYunque service action failed."
              : publicGenerationErrorMessage(error),
          },
        ],
        isError: true,
