...
async function accountPreflightResponse(
  input: Request | URL,
  init?: RequestInit,
) {
...
function directVideoParameters(body: Record<string, unknown>) {
  ...
  const directCall = JSON.parse(String(content?.data)) as Record<
    string,
    unknown
  >;
...
function call(
  overrides: Partial<GenerationCall> &
    Pick<GenerationCall, "operation_id" | "output" | "output_directory">,
): GenerationCall {
  return {
...
describe("XiaoYunque generation engine", () => {
  test("maps image/video inputs, downloads verified artifacts, and reuses operation ids", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-engine-"),
    );
    ...
    try {
      ...
      expect(
        (await readdir(outputOne)).some((name) => name.endsWith(".download")),
      ).toBeFalse();
      ...
      const imageContent = imageMessage.content as Array<
        Record<string, unknown>
      >;
      expect(imageContent).toHaveLength(1);
      expect(imageContent[0]).toMatchObject({
      ...
      await Promise.all([
        writeFile(firstFrame, png),
        writeFile(lastFrame, png),
        writeFile(audio, "RIFF-test"),
      ]);
      ...
      const video = await engine.generate(
        videoCall,
        visionVideoModel,
        new AbortController().signal,
      );
      ...
      expect(await operationStore.find("image-operation")).toMatchObject({
        status: "submitted",
      });
      expect(await operationStore.find("video-operation")).toMatchObject({
        status: "submitted",
      });
      ...
      for (const recordFile of recordFiles) {
        expect(
          (await lstat(path.join(recordDirectory, recordFile))).mode & 0o077,
        ).toBe(0);
      }
    } finally {
      server.stop(true);
  ...
  test("does not submit or persist an operation when reference image registration is rejected or incomplete", async () => {
    ...
    for (const registrationCase of cases) {
      const directory = await mkdtemp(
        path.join(
          os.tmpdir(),
          `xiaoyunque-reference-${registrationCase.name}-`,
        ),
      );
      directories.push(directory);
  ...
  test("registers and submits one Canvas reference image for video generation", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-first-frame-video-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    const firstFrame = path.join(directory, "first.png");
    ...
    const server = Bun.serve({
      ...
      fetch: async (request): Promise<Response> => {
        ...
        if (url.pathname === "/api/web/v1/common/upload_file") {
          ...
          const form = await request.formData();
          expect(
            path.basename((form.get("file") as File | null)?.name ?? ""),
          ).toBe("first.png");
        ...
        if (url.pathname === "/api/biz/v1/asset/create_v2") {
          ...
          return Response.json({
            data: { PippitAssetID: "pippit-first-frame" },
          });
        }
        const accountPreflight = await accountPreflightResponse(request);
        if (accountPreflight) return accountPreflight;
    ...
    try {
      ...
      expect(await store.find("single-first-frame-video")).toMatchObject({
        status: "submitted",
      });
    } finally {
      server.stop(true);
  ...
  test("registers and submits one Canvas reference video for video generation", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-reference-video-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    ...
    const server = Bun.serve({
      ...
      fetch: async (request): Promise<Response> => {
        ...
        if (url.pathname === "/api/biz/v1/asset/create_v2") {
          ...
          return Response.json({
            ret: 0,
            data: { PippitAssetID: "pippit-reference-video" },
          });
        }
        const accountPreflight = await accountPreflightResponse(request);
        if (accountPreflight) return accountPreflight;
    ...
    try {
      ...
      expect(await store.find("reference-video-generation")).toMatchObject({
        status: "submitted",
      });
    } finally {
      server.stop(true);
  ...
  test("rejects a last frame without a first frame before any API request", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-last-frame-only-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    const lastFrame = path.join(directory, "last.png");
    ...
    const failure = await engine
      .generate(generation, videoModel, new AbortController().signal)
      .catch((error: unknown) => error);
    expect(failure).toBeInstanceOf(XiaoYunqueGenerationInputError);
    expect((failure as XiaoYunqueGenerationInputError).publicMessage).toBe(
      "A video last frame requires exactly one first frame.",
    );
  ...
  test("rejects an operation-id replay with changed content", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-replay-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
  ...
  test("rejects an operation-id replay with a different selected model", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-model-replay-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
    const generation = call({
      operation_id: "same-model-operation",
      output: "video",
      output_directory: output,
    });
    const store = new OperationStore(
  ...
  test("serializes concurrent calls for one operation and submits it only once", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-concurrent-replay-"),
    );
    directories.push(directory);
    const outputOne = path.join(directory, "output-one");
    const outputTwo = path.join(directory, "output-two");
    ...
    const server = Bun.serve({
      ...
      fetch: async (request) => {
        ...
        if (url.pathname === "/api/biz/v1/agent/submit_run") {
          submitCount += 1;
          submittedTask = taskFromSubmitBody(
            (await request.json()) as Record<string, unknown>,
          );
          expect(
            await new OperationStore(storePath).find(
              "same-concurrent-operation",
            ),
    ...
    try {
      ...
      expect(
        await new OperationStore(storePath).find("same-concurrent-operation"),
      ).toMatchObject({
        status: "submitted",
  ...
  test("fails closed on a submitting tombstone before authorization or API access", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-tombstone-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
    const generation = call({
      operation_id: "uncertain-submit",
      output: "image",
      output_directory: output,
    });
    const store = new OperationStore(
    ...
    await expect(
      engine.generate(generation, imageModel, new AbortController().signal),
  ...
  test("persists a paid task before observing cancellation after submit", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-post-submit-cancel-"),
    );
    directories.push(directory);
    const outputOne = path.join(directory, "output-one");
    const outputTwo = path.join(directory, "output-two");
    ...
    await expect(
      engine.generate(generation, imageModel, controller.signal),
  ...
  test("keeps a submitted operation through request timeouts and long-running pending states", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-long-poll-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
  ...
  test("distinguishes a rejected status observation from a rejected submission", async () => {
    ...
    const failure = await engine
      .generate(generation, imageModel, new AbortController().signal)
      .catch((error: unknown) => error);

    expect(failure).toBeInstanceOf(XiaoYunqueObservationRejectedError);
    expect(
      (failure as XiaoYunqueObservationRejectedError).upstreamDiagnosticCode,
    ...
    expect(submitStageFailure).not.toBeInstanceOf(
      XiaoYunqueObservationRejectedError,
    );
    expect(submitStageQueryCount).toBe(0);
    expect(await store.find(submitStageCall.operation_id)).toMatchObject({
      status: "submitting",
    });
  });
  ...
  for (const terminal of [
    ...
    {
      message: "XiaoYunque video generation requires additional input",
      state: "6",
    },
    {
      message: "XiaoYunque video generation was interrupted for human input",
      state: "9",
    },
  ] as const) {
    test(`does not retry video terminal state ${terminal.state}`, async () => {
      const directory = await mkdtemp(
        path.join(os.tmpdir(), `xiaoyunque-video-terminal-${terminal.state}-`),
      );
      directories.push(directory);
      const output = path.join(directory, "output");
      await mkdir(output);
                ...
                run_list: [
                  {
                    entry_list: [],
                    ...(terminal.state === "4"
                      ? { fail_reason: { message: terminal.message } }
                      : {}),
                    run_id: submittedTask?.runId,
      ...
      expect(failure).toBeInstanceOf(XiaoYunqueTerminalGenerationError);
      expect((failure as XiaoYunqueTerminalGenerationError).reason).toBe(
        terminal.state === "4"
          ? "failed"
          : terminal.state === "5"
            ? "cancelled"
      ...
      expect((failure as Error).message).not.toContain("at the vendor");
  ...
  test("turns the exact unsupported raw-image terminal diagnostic into a safe typed failure", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-unsupported-image-model-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
  ...
  for (const terminal of [
    {
      message: "XiaoYunque image generation requires additional input",
      state: 6,
    },
    {
      message: "XiaoYunque image generation was interrupted for human input",
      state: 9,
    },
  ] as const) {
    test(`fails fast when the image thread enters unsupported state ${terminal.state}`, async () => {
      const directory = await mkdtemp(
        path.join(os.tmpdir(), `xiaoyunque-image-terminal-${terminal.state}-`),
      );
      directories.push(directory);
      const output = path.join(directory, "output");
      await mkdir(output);
      ...
      expect(failure).toBeInstanceOf(XiaoYunqueTerminalGenerationError);
      expect((failure as XiaoYunqueTerminalGenerationError).reason).toBe(
        terminal.state === 6 ? "additional-input" : "human-input",
      );
      expect(queryCount).toBe(1);
    });
  }
  ...
  test("removes the abort listener after a normal poll delay", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-poll-listener-"),
    );
    directories.push(directory);
    const output = path.join(directory, "output");
    await mkdir(output);
    ...
    ).rejects.toThrow("stop after delay");
