...
describe("MCP process", () => {
  ...
  test("fails fast with safe setup guidance and no API request when authorization is absent", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-no-auth-"),
    );
    ...
    try {
      ...
      const generated = await withTimeout(
        ...
        2_000,
        "missing authorization did not fail fast",
      );
      expect(generated).toMatchObject({ result: { isError: true } });
      const text =
        (generated.result as { content: Array<{ text: string }> }).content[0]
          ?.text ?? "";
  ...
  test("round-trips host browser authorization without exposing or auto-fetching Cookies", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-service-"),
    );
    directories.push(directory);
    const configRoot = path.join(directory, "config");
    const stateDirectory = path.join(configRoot, "convax", "xiaoyunque");
    ...
    const api = Bun.serve({
      ...
      fetch: (request) => {
        ...
        expect(request.headers.get("cookie")).toBe(
          `sessionid_pippitcn_web=${privateCookie}`,
        );
    ...
    try {
      ...
      const authorization = await harness.request("tools/call", {
      ...
      expect(authorization).toMatchObject({
        result: {
          content: [
            {
              text: "XiaoYunque browser authorization requested from the host.",
              type: "text",
            },
          ],
          structuredContent: {
      ...
      expect(completed).toMatchObject({
        result: {
          content: [
            {
              text: "XiaoYunque browser authorization stored locally.",
              type: "text",
            },
          ],
          structuredContent: {
      ...
      const stored = JSON.parse(await readFile(sessionPath, "utf8")) as Record<
        string,
        unknown
      >;
      ...
      const signedOut = await harness.request("tools/call", {
      ...
      const disconnected = await harness.request("tools/call", {
        name: "service.status",
        arguments: {},
      });
  ...
  test("returns a safe input error for a last frame without a first frame", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-last-frame-only-"),
    );
    directories.push(directory);
    const configRoot = path.join(directory, "config");
    const stateDirectory = path.join(configRoot, "convax", "xiaoyunque");
      ...
      expect(generated).toMatchObject({
        result: {
          content: [
            {
              type: "text",
              text: "A video last frame requires exactly one first frame.",
            },
          ],
          isError: true,
        },
  ...
  test("handshakes, lists every model tool, and generates an image through the real stdio process", async () => {
    ...
    const api = Bun.serve({
      ...
      fetch: async (request) => {
        ...
        if (url.pathname.startsWith("/api/")) {
          expect(request.headers.get("cookie")).toBe(
            `sessionid_pippitcn_web=${cookieValue}`,
          );
          expect(request.headers.has("authorization")).toBeFalse();
        }
                          ...
                          content: [
                            {
                              ...
                              data: JSON.stringify({
                                image: { url: `${url.origin}/artifact.png` },
                              }),
                            },
                          ],
                        },
    ...
    try {
      ...
      const artifact = (
        generated.result as {
          structuredContent: { artifacts: Array<{ path: string }> };
        }
      ).structuredContent.artifacts[0]!;
      expect(
        new Uint8Array(
          await readFile(path.join(outputDirectory, artifact.path)),
        ),
      ).toEqual(png);
      expect(submitCount).toBe(1);
      expect(
        harness.messages.every(
  ...
  test("generates a video through the shared get-thread endpoint in the real stdio process", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-video-"),
    );
    directories.push(directory);
    const configRoot = path.join(directory, "config");
    const outputDirectory = path.join(directory, "output");
  ...
  test("aborts an inflight generation and exits when the MCP client closes stdin", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-eof-"),
    );
    directories.push(directory);
    const configRoot = path.join(directory, "config");
    const stateDirectory = path.join(configRoot, "convax", "xiaoyunque");
    ...
    try {
      ...
      if (child.exitCode === null && child.signalCode === null) {
        ...
        await withTimeout(
          exited,
          2_000,
          "MCP process did not stop during test cleanup",
        );
      }
      api.stop(true);
    }
  ...
  test("drains an inflight handler and exits cleanly on SIGTERM", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-mcp-sigterm-"),
    );
    directories.push(directory);
    const configRoot = path.join(directory, "config");
    const stateDirectory = path.join(configRoot, "convax", "xiaoyunque");
    ...
    try {
      ...
      expect(
        await withTimeout(
          exited,
          2_000,
          "MCP process did not exit after SIGTERM",
        ),
