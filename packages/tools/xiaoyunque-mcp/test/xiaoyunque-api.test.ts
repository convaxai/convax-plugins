...
function expectWebHeaders(headersInit: HeadersInit | undefined) {
  ...
  expect(headers.get("cookie")).toBe(
    "sessionid_pippitcn_web=private-cookie-value",
  );
...
async function captureVideoToolParam(options: VideoSubmitOptions) {
  ...
  const api = apiFor(async (url, init) => {
    ...
    const directCall = JSON.parse(body.message.content[0].data) as {
      param: string;
      tool_name: string;
    };
      ...
      content: [
        {
          sub_type: subType,
          data: JSON.stringify({
            [subType === "biz/x_data_image" ? "image" : "video"]: media,
          }),
        },
      ],
    },
...
describe("XiaoYunque first-party Web generation API", () => {
  test("uploads references with the Cookie session, Web headers, and media asset type", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-upload-"),
    );
  ...
  test("registers an uploaded EverPhoto image when upload_file returns a blank Pippit asset id", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-register-upload-"),
    );
    directories.push(directory);
    const referencePath = path.join(directory, "reference.png");
    await writeFile(
  ...
  test("classifies failed or incomplete reference image registration without leaking upstream details", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-register-failure-"),
    );
    directories.push(directory);
    const referencePath = path.join(directory, "reference.png");
    await writeFile(
    ...
    const rejected = apiFor((url) =>
      url.pathname === "/api/web/v1/common/upload_file"
        ? uploadResponse()
        : Response.json({
            ret: 4001,
            errmsg: "private upstream registration detail",
          }),
    );
    const incomplete = apiFor((url) =>
      url.pathname === "/api/web/v1/common/upload_file"
        ? uploadResponse()
  ...
  test("maps video and audio references to the first-party upload asset types", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-media-upload-"),
    );
    directories.push(directory);
  ...
  test("registers an uploaded EverPhoto video with the video AssetCreateV2 type", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-register-video-"),
    );
    directories.push(directory);
  ...
  test("classifies failed reference video registration without leaking upstream details", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "xiaoyunque-web-register-video-failure-"),
    );
    directories.push(directory);
    const referencePath = path.join(directory, "reference.mp4");
    await writeFile(
  ...
  test("submits one image through the first-party Canvas image flow", async () => {
    ...
    const api = apiFor(async (url, init) => {
      ...
      expect(url.pathname).toBe("/api/biz/v1/agent/submit_run");
      submitBabiParam = JSON.parse(
        url.searchParams.get("babi_param") ?? "null",
      );
  ...
  test("submits video through the current Canvas direct video-part tool", async () => {
    ...
    expect(
      Object.hasOwn(JSON.parse(directCall.param), "resolution"),
    ).toBeFalse();
  ...
  test("rejects unsupported Seedance 1.0 Fast inputs before submission", async () => {
    ...
    const cases: Array<{
      expected: string;
      options: Partial<VideoSubmitOptions>;
    }> = [
      {
            ...
            run_list: [
              {
                ...
                state: 3,
                entry_list: [
                  artifactEntry("biz/x_data_image", {
                    scene_urls: {
                      download: "https://cdn.example.test/download.png",
                    },
    ...
    const api = apiFor(async (url, init) => {
      ...
      return threadResponse({
        entry_list: [
          ...
          artifactEntry("biz/x_data_video", {
  ...
  test("classifies only the exact live unsupported raw-image terminal failure", async () => {
    const responses: Array<{
      ...
      },
      {
        ...
        run: {
          entry_list: [],
          fail_reason: {
            message: "unsupported image_model_name: private-vendor-value",
          },
          run_id: task.runId,
          state: 4,
          thread_id: task.threadId,
    ...
    responses.shift();
    const arbitrary = await api.query(
      task,
      responses[0]!.output,
      session,
      signal,
    ...
    responses.shift();
    const wrongCapability = await api.query(
      task,
      responses[0]!.output,
      session,
      signal,
  ...
  test("allows unfinished media only while a run is not complete", async () => {
    ...
    await expect(
      completed.query(task, "video", session, signal),
    ).rejects.toThrow("video artifact URL is invalid");

    const completedWithoutMedia = apiFor(() =>
      threadResponse({
        entry_list: [],
        run_id: task.runId,
        state: 3,
    ...
    expect(
      await completedWithoutMedia.query(task, "video", session, signal),
    ).toMatchObject({
    ...
    const unsafePending = apiFor(() =>
      threadResponse({
        entry_list: [
          artifactEntry("biz/x_data_video", {
            url: "file:///private/result.mp4",
          }),
        ],
        run_id: task.runId,
        state: 2,
    ...
    await expect(
      unsafePending.query(task, "video", session, signal),
    ).rejects.toThrow("video artifact URL is invalid");

    const structurallyInvalidPending = apiFor(() =>
      threadResponse({
    ...
    await expect(
      structurallyInvalidPending.query(task, "video", session, signal),
    ).rejects.toThrow("generation content is invalid");

    const malformedMediaPending = apiFor(() =>
      threadResponse({
        entry_list: [
          {
            artifact: {
    ...
    await expect(
      malformedMediaPending.query(task, "video", session, signal),
  ...
  test("bounds one generation status request and aborts the stalled HTTP exchange", async () => {
    ...
    const api = new XiaoYunqueApi("https://xyq.jianying.com", stalledFetch, {
      queryRequestTimeoutMs: 5,
    });

    const failure = await api
      .query(task, "video", session, signal)
  ...
  test("classifies HTTP 401 and ret 1015 without exposing response content", async () => {
    const httpUnauthorized = apiFor(
      () => new Response("private response body", { status: 401 }),
    );
    await expect(
      httpUnauthorized.query(task, "video", session, signal),
    ).rejects.toBeInstanceOf(XiaoYunqueAuthenticationError);

    const retUnauthorized = apiFor(() =>
      Response.json({
        ret: 1015,
  ...
  test("rejects mismatched tasks, invalid artifacts, and oversized JSON without response leakage", async () => {
    ...
    await expect(
      mismatch.query(task, "video", session, signal),
    ).rejects.toThrow("does not match");

    const malformedArtifact = apiFor(() =>
      threadResponse({
        entry_list: [
          artifactEntry("biz/x_data_video", {
            url: "file:///private/result.mp4",
    ...
    await expect(
      malformedArtifact.query(task, "video", session, signal),
    ).rejects.toThrow("artifact URL is invalid");

    const oversized = apiFor(
      () =>
        new Response("{}", {
  ...
  test("rejects malformed video states and non-success envelopes without diagnostic leakage", async () => {
    ...
    await expect(
      malformedState.query(task, "video", session, signal),
    ).rejects.toThrow("generation state is invalid");

    const rejected = apiFor(() =>
      Response.json({
        ret: 4001,
    ...
    try {
      ...
      expect((error as XiaoYunqueRequestRejectedError).diagnosticCode).toBe(
    ...
    const httpFailure = await httpRejected
      .query(task, "video", session, signal)
      .catch((error: unknown) => error);
    expect(httpFailure).toBeInstanceOf(XiaoYunqueRequestRejectedError);
    expect((httpFailure as XiaoYunqueRequestRejectedError).diagnosticCode).toBe(
    ...
    for (const response of [
      ...
      const nonJsonFailure = await apiFor(() => response.clone())
        .query(task, "video", session, signal)
        .catch((error: unknown) => error);
      expect(nonJsonFailure).toBeInstanceOf(XiaoYunqueRequestRejectedError);
      expect(
        (nonJsonFailure as XiaoYunqueRequestRejectedError).diagnosticCode,
      ).toBe("upstream-http-rejected");
      expect(String(nonJsonFailure)).not.toContain("private upstream failure");
    }
  });
  ...
  test("keeps a real loopback fake API available without weakening production origins", async () => {
    ...
    expect(await api.query(task, "video", session, signal)).toMatchObject({
      state: 3,
    });
    expect(receivedCookie).toBe("sessionid_pippitcn_web=private-cookie-value");
    expect(() => new XiaoYunqueApi("https://attacker.example.test")).toThrow(
      "origin is invalid",
    );
  });
});
