...
const outputDirectory = await mkdtemp(
  path.join(os.tmpdir(), "convax-xiaoyunque-live-"),
);
const prompt =
  process.argv.slice(2).join(" ").trim() ||
  "一只由彩纸拼贴而成的小云雀，米白背景，柔和自然光，正方形构图";
...
try {
  const initialized = await request("initialize", {
    capabilities: {},
    clientInfo: { name: "convax-xiaoyunque-live-smoke", version: "0.3.4" },
  ...
  if (!names.includes("image.seedream_5.0")) {
    throw new Error(
      "Sidecar did not expose the default Seedream 5.0 generation tool",
    );
  }
  process.stderr.write(
    "将使用 Convax 服务页授权的小云雀网页会话执行一次计费生图。\n",
  );
  ...
  if (!result || result.isError)
    throw new Error("XiaoYunque live image generation failed");
  const artifacts = result.structuredContent?.artifacts ?? [];
  if (artifacts.length !== 1) {
    throw new Error(
      `XiaoYunque live image generation returned ${artifacts.length} artifacts instead of exactly one`,
    );
  }
  const artifact = artifacts[0];
  if (!artifact?.path || !artifact.mimeType?.startsWith("image/")) {
    throw new Error(
      "XiaoYunque live image generation returned no image artifact",
    );
  }
  const artifactPath = path.join(outputDirectory, artifact.path);
  const bytes = await readFile(artifactPath);
  if (bytes.byteLength < 8)
    throw new Error("XiaoYunque live image artifact is empty");
