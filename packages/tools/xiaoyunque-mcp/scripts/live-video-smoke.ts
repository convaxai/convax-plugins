...
const outputDirectory = await mkdtemp(
  path.join(os.tmpdir(), "convax-xiaoyunque-live-video-"),
);
const prompt =
...
let resolveGenerationFailureCategory:
  | ((category: SafeGenerationDiagnosticCode) => void)
  | undefined;
const generationFailureCategoryObserved =
...
const childExit = new Promise<void>((resolve) =>
  child.once("exit", () => {
    for (const request of pending.values()) {
      request.reject(
        new Error(
          "XiaoYunque sidecar exited before the live video smoke completed",
        ),
      );
    }
    resolve();
...
async function generationFailureCategory(): Promise<SafeGenerationDiagnosticCode> {
  if (observedGenerationFailureCategory)
    return observedGenerationFailureCategory;
...
try {
  ...
  const initialized = await request("initialize", {
    capabilities: {},
    clientInfo: {
      name: "convax-xiaoyunque-live-video-smoke",
      version: "0.3.4",
    },
    protocolVersion: "2025-03-26",
  });
  if (
    (initialized.result as Record<string, unknown> | undefined)
      ?.protocolVersion !== "2025-03-26"
  ) {
  ...
  if (!names.includes(toolName)) {
    throw new Error(
      "Sidecar did not expose the default Seedance 2.0 Mini Lite generation tool",
    );
  }
  ...
  const result = generated.result as
    | {
        isError?: boolean;
        structuredContent?: {
          artifacts?: Array<{ mimeType?: string; path?: string }>;
        };
      }
    | undefined;
  ...
  if (artifacts.length !== 1) {
    throw new Error(
      `XiaoYunque live video generation returned ${artifacts.length} artifacts instead of exactly one`,
    );
  }
  const artifact = artifacts[0];
  if (!artifact?.path || !artifact.mimeType?.startsWith("video/")) {
    throw new Error(
      "XiaoYunque live video generation returned no video artifact",
    );
  }
  ...
  if (bytes.byteLength < 12)
    throw new Error("XiaoYunque live video artifact is empty");
