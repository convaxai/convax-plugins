...
Production requests resolve those fixed origins to IPv6 and connect to the
address while retaining the original HTTP Host and verified TLS server name.
This avoids managed networks that reset Cloudflare IPv4 TLS after accepting the
TCP connection; it never disables certificate verification or falls back to a
different authority.
