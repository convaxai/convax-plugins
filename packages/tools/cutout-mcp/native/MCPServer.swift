...
final class MCPServer {
  private let engine = CutoutEngine()
  ...
  private func receive(_ data: Data) {
    ...
    case "tools/list": sendResult(id: id, result: ["tools": [cutoutTool]])
  ...
  private func callTool(id: RPCID, params: Any?) {
    ...
    do {
      ...
      guard object["name"] as? String == "background.remove" else {
    ...
    DispatchQueue.global(qos: .userInitiated).async { [self] in
      ...
      do {
        let call = try parseGenerationCall(object["arguments"])
        let artifacts = try engine.generate(call, control: control)
        sendResult(id: id, result: [
          "content": [[
            "text": "Created one transparent PNG locally with U-2-Netp ONNX.",
            "type": "text",
          ]],
          "structuredContent": [
  ...
  private func cancel(_ params: Any?) {
    guard let object = params as? [String: Any],
          let id = RPCID.parse(object["requestId"])
    else { return }
    inflightLock.lock()
