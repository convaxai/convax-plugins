...
function assertPortablePluginStateValueV1(schemaInput, value) {
  ...
  validateValue2(schema, value, 1, { nodes: 0, seen: new Set });
  let bytes;
  try {
...
function parseSchema(value, depth, budget) {
  ...
  try {
    if (input.type === "null" || input.type === "boolean") {
      exactKeys4(input, ["type"]);
      return Object.freeze({ type: input.type });
    }
    if (input.type === "string") {
      exactKeys4(input, ["type", "maxUtf8Bytes"], ["enum"]);
    ...
    if (input.type === "integer") {
      exactKeys4(input, ["type", "minimum", "maximum"]);
    ...
    if (input.type === "array") {
      exactKeys4(input, ["type", "maxItems", "items"]);
      return Object.freeze({
        type: "array",
        maxItems: boundedDecimal(input.maxItems, maximumArrayItems2, "Array maxItems"),
        items: parseSchema(input.items, depth + 1, budget)
      });
    }
    if (input.type === "object") {
      exactKeys4(input, ["type", "maxProperties", "required", "properties", "additionalProperties"]);
      if (input.additionalProperties !== false)
    ...
    if (input.type === "union") {
      exactKeys4(input, ["type", "variants"]);
...
function validateValue2(schema, value, depth, budget) {
  ...
  if (schema.type === "string") {
    const text3 = normalizedText(value, "Plugin state string", Number(schema.maxUtf8Bytes));
    if (schema.enum !== undefined && !schema.enum.includes(text3))
  ...
  if (schema.type === "integer") {
    ...
    if (integer2 < BigInt(schema.minimum) || integer2 > BigInt(schema.maximum)) {
  ...
  if (schema.type === "union") {
    const matches = schema.variants.filter((variant) => {
      try {
        validateValue2(variant, value, depth + 1, { nodes: budget.nodes, seen: new Set(budget.seen) });
        return true;
      } catch {
        return false;
      }
  ...
  try {
    if (schema.type === "array") {
      ...
      for (const item of value)
        validateValue2(schema.items, item, depth + 1, budget);
      return;
    }
    if (Array.isArray(value))
    ...
    for (const key of keys) {
      ...
      validateValue2(child, input[key], depth + 1, budget);
    }
  } finally {
...
function safeIntegerDecimal(value, label) {
  ...
  const integer2 = BigInt(value);
  if (integer2 < BigInt(Number.MIN_SAFE_INTEGER) || integer2 > BigInt(Number.MAX_SAFE_INTEGER)) {
...
//# debugId=9B556CBB54ED7F9D64756E2164756E21
