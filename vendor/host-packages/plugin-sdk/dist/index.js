...
var windowsReservedName = /^(CON|PRN|AUX|NUL|COM[1-9¹²³]|LPT[1-9¹²³]|CONIN\$|CONOUT\$)$/i;
...
function validatePortablePluginSegment(value) {
  ...
  if (!value || value.length > 255 || value === "." || value === ".." || /[\\/:*?"<>|\u0000-\u001f\u007f]/u.test(value) || /[. ]$/u.test(value) || windowsReservedName.test(stem)) {
...
var semverPattern2 = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/;
...
function text2(value, label, maximum = 2000) {
  if (typeof value !== "string" || value.length < 1 || value.length > maximum || value !== value.trim() || /[\u0000-\u001f\u007f]/u.test(value)) {
    throw new TypeError(`${label} must be a bounded, trimmed string`);
  }
  return value;
...
function version(value, label) {
  if (typeof value !== "string" || !semverPattern2.test(value)) {
...
function normalizeExport(value, label) {
  ...
  if (!isPluginCapabilityId(id))
    throw new TypeError(`${label}.id is invalid`);
  const operation = text2(input.operation, `${label}.operation`, 128);
  ...
  const docs = Object.freeze({
    ...
    response: text2(rawDocs.response, `${label}.docs.response`),
    ...rawDocs.remarks === undefined ? {} : { remarks: text2(rawDocs.remarks, `${label}.docs.remarks`) }
  });
  return Object.freeze({
    id,
...
function assertPluginCapabilityRuntimeTools(exports, tools) {
  ...
  for (const tool of tools) {
    const name = text2(tool.name, "Runtime MCP tool name", 128);
...
function parseAgentRemoteMcp(value) {
  ...
  if (input.headers !== undefined) {
    ...
    for (const [name, value2] of entries) {
      ...
      const literal = portableText(value2, `Agent remote MCP header ${name}`, 2048);
...
function definePluginApiRelease(version2, apis) {
  assertVersion(version2, "Plugin API release version");
  return Object.freeze({ version: version2, apis: Object.freeze([...apis]) });
}
function definePluginApiCatalog(...releases) {
  ...
  for (const release of releases) {
    ...
    if (previous && compareVersions2(previous, release.version) >= 0) {
...
var windowsReservedName2 = /^(CON|PRN|AUX|NUL|COM[1-9¹²³]|LPT[1-9¹²³]|CONIN\$|CONOUT\$)$/iu;
...
function isPortableNameSegment(value) {
  ...
  return Boolean(value && value !== "." && value !== ".." && hasOnlyUnicodeScalars(value) && !/[\\/:*?"<>|\u0000-\u001f\u007f]/u.test(value) && !/[. ]$/u.test(value) && !windowsReservedName2.test(stem));
}
...
function json(value, schema, label) {
  ...
  const result = visit(record3(value, label), label, 0);
