...
async function writeFixture() {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "convax-product-lock-output-"),
  )
  ...
  const standaloneSkillTag = "skill-writing-workflow-v0.1.0"
  const builtinTag = `builtin-${"b".repeat(64)}`
  const plugin = await writeRelease(
    root,
    "catalog",
    pluginTag,
  ...
  const skill = await writeRelease(
    root,
    "catalog",
    skillTag,
    "convax-skill-ffmpeg-canvas-0.3.1.zip",
    Buffer.from("skill"),
  )
  const standaloneSkill = await writeRelease(
    root,
    "catalog",
    standaloneSkillTag,
    "convax-skill-writing-workflow-0.1.0.zip",
    Buffer.from("standalone-skill"),
  )
  const companion = await writeRelease(
    root,
    "catalog",
    pluginTag,
  ...
  const descriptorValue = {
    ...
    registry: {
      v2: {
        url: "https://convaxai.github.io/convax-plugins/registry/v2/index.json",
      },
    },
    showcase: {
      v2: {
        url: "https://convaxai.github.io/convax-plugins/showcase/v2/index.json",
      },
    },
    ...
    packages: [
      {
        ...
        manifest: {
          contributes: {
            skills: [{ name: "ffmpeg-canvas", path: "skills/ffmpeg-canvas" }],
          },
        },
      },
      {
      ...
      },
      {
        ...
        delivery: {
          kind: "artifact",
          url: standaloneSkill.url,
          size: standaloneSkill.size,
          sha256: standaloneSkill.sha256,
        },
      },
    ],
  }
  ...
  for (const [name, value] of [
    ...
    metadata[name] = await writeRelease(
      root,
      "catalog",
      metadataTag,
      name,
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({
      ...
      official: {
        descriptor: {
          path: metadata["marketplace.json"].path,
          url: metadata["marketplace.json"].url,
        },
        registry: {
          path: metadata["registry-v2.json"].path,
          url: metadata["registry-v2.json"].url,
        },
        revision,
        showcase: {
          path: metadata["showcase-v2.json"].path,
          url: metadata["showcase-v2.json"].url,
        },
      },
      packages: [
        {
        ...
        },
        {
          ...
          version: "0.1.0",
          setup: "none",
          artifact: { path: standaloneSkill.path, url: standaloneSkill.url },
          ownedSkills: [],
          companions: [],
        },
      ],
...
describe("Convax product lock input closure", () => {
  test("closes the Builtin reservation, Official metadata, Plugin closure, and standalone Skill", async () => {
    const fixture = await writeFixture()
    await expect(
      verifyProductLockInput(
        path.join(fixture.root, "product-lock-input.json"),
      ),
    ).resolves.toEqual({
  ...
  test("verifies an existing packaged artifact outside a selected package release-plan", async () => {
    const fixture = await writeFixture()
    ...
    const pluginBytes = await fs.readFile(
      path.join(fixture.root, originalPluginPath),
    )
    const inheritedPluginPath = `catalog/inherited/${sha256(pluginBytes)}/${path.basename(originalPluginPath)}`
    await fs.mkdir(path.dirname(path.join(fixture.root, inheritedPluginPath)), {
      recursive: true,
    })
    await fs.writeFile(
      path.join(fixture.root, inheritedPluginPath),
      pluginBytes,
    )
    ...
    await expect(verifyProductLockInput(lockPath)).rejects.toThrow(
      "packaged Plugin artifact bytes differ from Registry v2",
    )
  })

  test("rejects a source swap or widened packaged closure", async () => {
    const fixture = await writeFixture()
    const lockPath = path.join(fixture.root, "product-lock-input.json")
    const lock = JSON.parse(await fs.readFile(lockPath, "utf8"))
    ...
    await fs.writeFile(lockPath, `${JSON.stringify(lock)}\n`)
    await expect(verifyProductLockInput(lockPath)).rejects.toThrow(
      "packaged Plugin artifact differs from Registry v2",
    )
    ...
    await expect(verifyProductLockInput(cleanPath)).rejects.toThrow(
      "product lock package identities must be unique",
    )
  })
  ...
  test("rejects metadata or Builtin paths that escape the composed output", async () => {
    ...
    await fs.writeFile(lockPath, `${JSON.stringify(lock)}\n`)
    await expect(verifyProductLockInput(lockPath)).rejects.toThrow(
      "must stay below the product lock output",
    )
  })
  ...
  test("rejects filesystem aliases and oversized immutable inputs before hashing", async () => {
    ...
    await fs.link(
      descriptor,
      path.join(hardlinked.root, "descriptor-hardlink.json"),
    )
    await expect(
      verifyProductLockInput(
        path.join(hardlinked.root, "product-lock-input.json"),
      ),
    ...
    await expect(
      verifyProductLockInput(
        path.join(symlinked.root, "product-lock-input.json"),
      ),
    ).rejects.toThrow("single-link regular no-follow file")
    ...
    await expect(
      verifyProductLockInput(
        path.join(oversized.root, "product-lock-input.json"),
      ),
