...
import { parseAcceptedApiContracts } from "./host-capability-api-contracts.mjs"
...
function parsePublication(value, label) {
  ...
  const blockerKeys = blockers.map((item) => `${item.code}\0${item.note}`)
...
const hostCapabilityPolicySchema = "convax.host-capability-policy/3"
const hostContractVerificationModes = new Set([
  "catalog-contracts",
  "package-conformance",
])
const currentHostApiContracts = new Map(
  JSON.parse(renderPluginApiJson()).apis.map((api) => [
    api.id,
    api.contract.digest,
  ]),
)
...
export function assertPluginHostCapabilityDeclarations(
  ...
  for (const [required, requestId, reason] of requiredRequests) {
    if (required && !declared.has(requestId)) {
      error(
        label,
        `${reason}; declare automated Host contract requirement ${requestId}`,
      )
    }
  }
}

function parsePolicyAffectedIdentity(item, label) {
  exactKeys(item, ["id", "kind", "version"], ["id", "kind", "version"], label)
  if (item.kind !== "plugin" && item.kind !== "skill") {
    error(label, "kind must be plugin or skill")
  }
  return {
    kind: item.kind,
    id: parseId(item.id, `${label} id`),
    version: parseSemver(item.version, `${label} version`),
  }
}

function parsePolicyId(value, label) {
  const id = cleanString(value, label, 128)
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id)) {
    error(label, "must be a lowercase kebab-case identifier")
  }
  return id
}

function sortAffected(left, right) {
...
export function parseHostCapabilityPolicy(
  ...
  if (value.schema !== hostCapabilityPolicySchema)
    error(label, "unsupported schema")
  if (!Array.isArray(value.requirements) || value.requirements.length > 1_000) {
    error(label, "requirements must be an array with at most 1000 items")
  }
  const requirements = value.requirements
    .map((requirement, requirementIndex) => {
      const requirementLabel = `${label} requirement ${requirementIndex}`
      exactKeys(
        requirement,
      ...
      const id = parsePolicyId(requirement.id, `${requirementLabel} id`)
      if (!hostContractVerificationModes.has(requirement.verification)) {
        error(
          requirementLabel,
          "verification must be catalog-contracts or package-conformance",
        )
      }
      ...
      if (
        ...
        error(
          requirementLabel,
          "catalog-contracts verification requires at least one accepted API contract",
        )
      }
      if (
        ...
        error(
          requirementLabel,
          "package-conformance verification must not duplicate API contracts",
        )
      }
      for (const contract of acceptedApiContracts) {
        const currentDigest = currentHostApiContracts.get(contract.id)
        if (currentDigest !== contract.digest) {
          error(
            requirementLabel,
            `Host API ${contract.id} contract digest does not match the current Catalog`,
          )
        }
      }
      if (
        ...
        error(
          requirementLabel,
          "affected must contain from 1 to 1000 package versions",
        )
      }
      const affected = requirement.affected.map((item, itemIndex) => {
        const identity = parsePolicyAffectedIdentity(
          item,
        ...
        if (
          ...
          error(
            requirementLabel,
            "catalog-contracts verification may affect only Plugins with manifests",
          )
        }
        return identity
      })
      ...
      if (new Set(affectedIdentities).size !== affectedIdentities.length) {
        error(requirementLabel, "contains duplicate affected package versions")
      }
      return {
        id,
        verification: requirement.verification,
        acceptedApiContracts,
        affected: affected.sort(sortAffected),
      }
    })
    .sort((left, right) => left.id.localeCompare(right.id, "en"))
  const requirementIds = requirements.map((requirement) => requirement.id)
  if (new Set(requirementIds).size !== requirementIds.length) {
    error(label, "contains duplicate requirement ids")
  }
  const requirementsByPackageVersion = new Map()
  for (const requirement of requirements) {
    for (const item of requirement.affected) {
      const identity = `${item.kind}/${item.id}@${item.version}`
      const ids = requirementsByPackageVersion.get(identity) ?? []
      ids.push(requirement.id)
      requirementsByPackageVersion.set(identity, ids)
    }
  }
  for (const [identity, ids] of requirementsByPackageVersion) {
    if (ids.length > maximumHostCapabilityRequestsPerPackage) {
      error(
        label,
        `${identity} binds more than ${maximumHostCapabilityRequestsPerPackage} automated requirements`,
      )
    }
  }
  if (!Array.isArray(value.blockers) || value.blockers.length > 1_000) {
    error(label, "blockers must be an array with at most 1000 items")
  }
  const blockers = value.blockers
    .map((policy, policyIndex) => {
      const policyLabel = `${label} blocker policy ${policyIndex}`
      exactKeys(policy, ["affected", "id"], ["affected", "id"], policyLabel)
      const id = parsePolicyId(policy.id, `${policyLabel} id`)
      if (
        !Array.isArray(policy.affected) ||
        policy.affected.length < 1 ||
        policy.affected.length > 1_000
      ) {
        error(
          policyLabel,
          "affected must contain from 1 to 1000 package versions",
      ...
      const affected = policy.affected.map((item, itemIndex) => {
        const itemLabel = `${policyLabel} affected ${itemIndex}`
        exactKeys(
          item,
      ...
      if (new Set(identities).size !== identities.length) {
        error(policyLabel, "contains duplicate affected package versions")
      }
      return { id, affected: affected.sort(sortAffected) }
    })
    .sort((left, right) => left.id.localeCompare(right.id, "en"))
  const blockerIds = blockers.map((policy) => policy.id)
  if (new Set(blockerIds).size !== blockerIds.length) {
    error(label, "contains duplicate blocker policy ids")
  }
  const packages = blockers
    .flatMap((policy) =>
      policy.affected.map((item) => ({ ...item, policyId: policy.id })),
    )
    .sort((left, right) =>
      `${left.kind}/${left.id}@${left.version}\0${left.policyId}`.localeCompare(
        `${right.kind}/${right.id}@${right.version}\0${right.policyId}`,
        "en",
      ),
    )
  return {
...
export async function loadPublicationPolicy(workspaceRoot = root) {
  const label = "Automated publication policy"
  ...
  const requirementsLabel = "Automated Host contract workspace declarations"
  ...
  for (const [kind, directoryName] of [
    ...
    for (const entry of await fs
      ...
      for (const value of declarations) {
        ...
        const affected = declarationsByRequirement.get(requestId) ?? []
        affected.push(identity)
        declarationsByRequirement.set(requestId, affected)
      }
    }
  }
  ...
  for (const requirementId of new Set([
    ...declarationsByRequirement.keys(),
    ...policyByRequirement.keys(),
  ])) {
    const declared = (declarationsByRequirement.get(requirementId) ?? []).sort()
    const affected = policyByRequirement.get(requirementId)
    if (!affected) {
      error(
        requirementsLabel,
        `declared automated requirement ${requirementId} is missing from publication policy`,
      )
    }
    if (
      ...
      error(
        requirementsLabel,
        `automated requirement ${requirementId} must exactly match workspace declarations and policy affected versions`,
      )
    }
  }
  return policy
...
function assertPortableWebReference(
  reference,
  sourcePath,
  packagePaths,
  label,
  ...
  if (!packagePaths.has(resolved)) {
    error(
      label,
      `Web subresource URL does not resolve to a package file: ${value}`,
    )
  }
}
...
export async function discoverPackages(options = {}) {
  ...
  const candidates = (await discoverMarketplacePackages(workspaceRoot))
    .filter(
      (candidate) => candidate.kind === "plugin" || candidate.kind === "skill",
    )
    .map((candidate) => ({
      ...candidate,
  ...
  const contractRequirementsByVersion = new Map()
  for (const requirement of publicationPolicy.requirements) {
    if (requirement.verification !== "catalog-contracts") continue
    for (const affected of requirement.affected) {
      const identity = `${affected.kind}/${affected.id}@${affected.version}`
      const current = contractRequirementsByVersion.get(identity) ?? []
      current.push(...requirement.acceptedApiContracts)
      contractRequirementsByVersion.set(identity, current)
    }
  }
      ...
      parsePublication(
        {
          ...
          blockers: [
            ...
            ...item.blockers.map((blocker) => ({
              ...blocker,
              note: `[${item.policyId}] ${blocker.note}`,
            })),
          ],
        },
  ...
  async function loadCandidate(candidate) {
    ...
    const metadata = {
      ...candidate.authoring,
      publication: publicationByVersion.get(
    ...
    if (candidate.kind === "plugin") {
      ...
      if (!manifest)
        error(
          `${candidate.kind}/${candidate.id}`,
          "missing canonical SDK manifest",
        )
      assertPluginStatic(
        files,
        `${candidate.kind}/${candidate.id}`,
      ...
      if (manifest.schema !== "convax.plugin/8") {
        error(
          `${candidate.kind}/${candidate.id}`,
          "only convax.plugin/8 is publishable",
        )
      }
      ...
      const declaredHostApis = new Set([
        ...manifest.hostApi.required,
        ...manifest.hostApi.optional,
      ])
      for (const contract of contractRequirementsByVersion.get(
        `plugin/${candidate.id}@${candidate.version}`,
      ) ?? []) {
        if (!declaredHostApis.has(contract.id)) {
          error(
            `plugin/${candidate.id}`,
            `automated Host contract requirement is not declared by the manifest: ${contract.id}`,
          )
        }
      }
