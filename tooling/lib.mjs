...
const pluginProjectCanvasCapabilities = new Set(["projects.read", "canvas.catalog.read", "canvas.document.read", "canvas.document.write", "canvas.events.subscribe"])
const pluginPetCapabilities = new Set(["pet.activity.read", "pet.activity.open", "pet.preferences.write", "pet.custom.manage"])
const pluginCapabilitiesV5 = new Set([...pluginCapabilitiesV4, ...pluginProjectCanvasCapabilities, ...pluginPetCapabilities])
const pluginCapabilitiesV6 = new Set([...pluginCapabilitiesV4, "canvas.resources.write", ...pluginProjectCanvasCapabilities, "canvas.connectedInputs.read"])
const pluginCapabilities = pluginCapabilitiesV5
const pluginV5Capabilities = new Set([...pluginProjectCanvasCapabilities, ...pluginPetCapabilities])
const generationModalities = new Set(["text", "image", "video", "audio"])
const generationInputRoles = new Set(["reference_image", "reference_video", "first_frame", "last_frame", "audio", "text"])
...
function inspectHookModuleSource(source, label, relativePath) {
  ...
  const hasPluginExport = program.body.some(
    (statement) => statement.type === "ExportDefaultDeclaration" || (statement.type === "ExportNamedDeclaration" && (statement.declaration !== null || statement.specifiers.length > 0)),
  )
  const staticImports = []
  ...
  while (pending.length > 0) {
    ...
    if (node.type === "Identifier" && ["exports", "module", "require"].includes(node.name)) {
...
function cleanString(value, label, maxLength) {
  if (typeof value !== "string" || value !== value.trim() || value.length === 0 || value.length > maxLength || /[\u0000-\u001f\u007f]/.test(value)) {
...
function parseShowcaseSource(value, label) {
  ...
  const animation = value.animation === undefined ? undefined : parseShowcaseSourceMedia(value.animation, "animation", `${label} animation`)
...
export function validatePortableSegment(value, label = "path") {
  ...
  if (!value || value.length > 255 || value === "." || value === ".." || /[\\/:*?"<>|\u0000-\u001f\u007f]/.test(value) || /[. ]$/.test(value) || windowsReservedName.test(stem))
...
function parseHookModule(value, label) {
  ...
  if (!/\.(?:js|mjs)$/.test(result)) error(label, "must be a JavaScript ESM module")
  return result
}
...
function parseCompatibility(value, kind, label) {
  if (kind === "plugin") {
    ...
    const v6 = value.pluginSchema === "convax.plugin/6" && value.pluginHost === "convax.plugin-capability/1"
    if (!v1 && !v2 && !v3 && !v4 && !v5 && !v6) {
      error(label, "must pair matching convax.plugin and convax.plugin-host major versions 1-4, or pair convax.plugin/5-6 with convax.plugin-capability/1")
    }
...
function parseSourceCompanions(value, label) {
  ...
  const companions = value.map((item, index) => {
    ...
    exactKeys(item, ["command", "source", "targets", "version"], ["command", "source", "targets", "version"], itemLabel)
...
export function parseSourceMetadata(value, label = "convax-package.json") {
  ...
  const ownerPluginId = value.ownerPluginId === undefined ? undefined : parseId(value.ownerPluginId, `${label} ownerPluginId`)
...
function parseLegacyPluginManifest(value, label) {
  ...
  exactKeys(
    value,
    ["capabilities", "contributes", "description", "entry", "hooks", "id", "name", ...(v2 ? ["runtime"] : []), "schema", "skill", "version"],
  ...
  if (
    ...
    capabilities.some((item) => typeof item !== "string" || !pluginCapabilitiesV4.has(item)) ||
  ...
  if (v2 && !hasRuntime && hooks === undefined && !capabilities.includes("generation.execute")) {
...
function parseLocalizedText(value, label, maxLength) {
  ...
  return {
    ...
    ...(value["zh-CN"] === undefined ? {} : { "zh-CN": cleanString(value["zh-CN"], `${label} zh-CN`, maxLength) }),
  }
}
...
function parseGenerationV3(value, label, options = {}) {
  ...
  const returnedModel = models.find((model) => tools.find((tool) => tool.id === model.tool)?.delivery === "return")
  if (returnedModel) {
    error(label, `model ${returnedModel.name} cannot reference return-delivery operation ${returnedModel.tool}`)
  }
  const inputBoundModel = models.find((model) => tools.find((tool) => tool.id === model.tool)?.inputBinding === "direct-incoming")
  if (inputBoundModel) {
    error(label, `model ${inputBoundModel.name} cannot reference direct-incoming operation ${inputBoundModel.tool}`)
  }
  return { models, tools }
}
...
function parseSelectionActionsV3(value, generation, label) {
  ...
  const actions = value.map((item, index) => {
    const itemLabel = `${label} item ${index}`
    exactKeys(item, ["description", "editor", "id", "steps", "target", "title"], ["description", "editor", "id", "steps", "target", "title"], itemLabel)
    ...
    const returned = steps.find((step) => generationTools.get(step.tool).delivery === "return")
    if (returned) error(itemLabel, `cannot reference return-delivery operation ${returned.tool}`)
...
function parseCanvasV3(value, generation, label) {
  ...
  const selectionActions = value.selectionActions === undefined ? undefined : parseSelectionActionsV3(value.selectionActions, generation, `${label} selectionActions`)
...
function parsePluginManifestV3(value, label) {
  const required = ["contributes", "description", "id", "name", "schema", "version"]
  exactKeys(value, ["capabilities", "contributes", "description", "entry", "hooks", "id", "name", "runtime", "schema", "skill", "version"], required, label)
  ...
  if (!hasRuntime && hooks === undefined && !capabilities.includes("generation.execute")) {
  ...
  const agent = value.contributes.agent === undefined ? undefined : parseAgentV3(value.contributes.agent, generation, `${label} agent`)
  ...
  const canvas = hasCanvas ? parseCanvasV3(value.contributes.canvas, generation, `${label} canvas`) : undefined
...
function parseAgentRemoteMcpV6(value, label) {
  ...
  try {
    ...
    error(label, "url must be an absolute HTTPS URL without credentials or a fragment")
  }
  if (value.oauth !== undefined && value.oauth !== "auto" && value.oauth !== "none") {
    error(label, "oauth must be auto or none")
  }
  let headers
  if (value.headers !== undefined) {
    if (!isObject(value.headers)) error(`${label} headers`, "must be an object")
    const entries = Object.entries(value.headers)
    if (entries.length > 16) error(`${label} headers`, "must contain at most 16 entries")
...
function parseAgentV6(value, generation, label) {
  ...
  const tools = value.tools === undefined ? undefined : parseAgentV3({ tools: value.tools }, generation, label).tools
  const mcp = value.mcp === undefined ? undefined : parseAgentRemoteMcpV6(value.mcp, `${label} mcp`)
  return {
    ...(mcp === undefined ? {} : { mcp }),
...
function parsePluginManifestV4Plus(value, label) {
  ...
  const v5OrLater = schema === "convax.plugin/5" || schema === "convax.plugin/6"
  const v6 = schema === "convax.plugin/6"
  const required = ["contributes", "description", "id", "name", "schema", "version"]
  exactKeys(value, ["capabilities", "contributes", "description", "entry", "hooks", "id", "name", "runtime", "schema", "version"], required, label)
  exactKeys(value.contributes, ["agent", "canvas", "generation", ...(v5OrLater ? ["llm"] : []), "service", "skills"], [], `${label} contributes`)

  const capabilities = value.capabilities ?? []
  const allowedCapabilities = v6 ? pluginCapabilitiesV6 : v5OrLater ? pluginCapabilitiesV5 : pluginCapabilitiesV4
  if (
    !Array.isArray(capabilities) ||
  ...
  const generation = hasGeneration
    ? parseGenerationV3(value.contributes.generation, `${label} generation`, {
        allowReturnDelivery: v6,
      })
    : undefined
  if (!v6 && value.contributes.agent !== undefined) {
    exactKeys(value.contributes.agent, ["tools"], ["tools"], `${label} agent`)
  }
  if (!v6 && value.contributes.agent !== undefined && generation === undefined) {
    error(label, "agent tools require a generation contribution")
  }
  const agent =
    value.contributes.agent === undefined ? undefined : v6 ? parseAgentV6(value.contributes.agent, generation, `${label} agent`) : parseAgentV3(value.contributes.agent, generation, `${label} agent`)
  ...
  const canvas = hasCanvas ? parseCanvasV3(value.contributes.canvas, generation, `${label} canvas`) : undefined
  const hasRenderer = canvas?.renderer !== undefined
  const hasProjectCanvasCapability = capabilities.some((capability) => pluginProjectCanvasCapabilities.has(capability))
  if (!hasRuntime && hooks === undefined && !hasRenderer && !capabilities.includes("generation.execute") && !hasProjectCanvasCapability && agent?.mcp === undefined) {
...
function parsePluginManifestV5(value, label) {
  const required = ["contributes", "description", "id", "name", "schema", "version"]
  exactKeys(value, ["capabilities", "contributes", "description", "entry", "hooks", "id", "name", "runtime", "schema", "version"], required, label)
  exactKeys(value.contributes, ["agent", "canvas", "generation", "llm", "pet", "service", "skills"], [], `${label} contributes`)
  ...
  const skills = parseOwnedSkillsV4(value.contributes.skills, `${label} skills`)
  const pet = value.contributes.pet === undefined ? undefined : parsePetV5(value.contributes.pet, `${label} pet`)
  if (pet !== undefined) {
  ...
  if (!hasRuntime && !hasRenderer && !canvas?.selectionActions?.length && hooks === undefined && !capabilities.includes("generation.execute") && !hasProjectCapability && pet === undefined) {
...
export function parsePluginManifest(value, label = "manifest.json") {
  ...
  if (value.schema === "convax.plugin/4" || value.schema === "convax.plugin/6") {
...
function parseMcpStdioRuntime(value, label) {
  ...
  if (value.args !== undefined) {
    ...
    args = value.args.map((item, index) => {
      ...
      if (/[\s"'`;|&`$(){}[\]<>]/.test(argument) || argument.includes("\\") || /(^|=)(?:\/|[A-Za-z]:)/.test(argument) || /(^|[=/])\.{1,2}(?:\/|$)/.test(argument)) {
...
function parseGeneration(value, label, options = {}) {
  ...
  const tools = value.tools.map((item, index) => {
    ...
    exactKeys(
      item,
      ["acceptedInputs", ...(options.allowReturnDelivery ? ["delivery", "inputBinding"] : []), "description", "id", "output", "title"],
    ...
    if (item.delivery !== undefined && item.delivery !== "canvas" && item.delivery !== "return") {
    ...
    if (item.inputBinding !== undefined && item.inputBinding !== "direct-incoming") {
    ...
    if (item.inputBinding === "direct-incoming" && item.acceptedInputs.length === 0) {
      error(itemLabel, "direct-incoming input binding requires accepted inputs")
    }
    return {
      ...
      id,
      ...(item.inputBinding === undefined ? {} : { inputBinding: item.inputBinding }),
      output: item.output,
      title: cleanString(item.title, `${itemLabel} title`, 120),
    }
  })
...
export function assertPluginStatic(files, label, hookPath) {
  if (hookPath !== undefined && !files.some((file) => file.relativePath === hookPath)) {
  ...
  for (const file of files) {
    ...
    if ([".html", ".css", ".js", ".mjs", ".cjs"].includes(extension)) {
      ...
      if (hookModule) {
        const inspection = inspectHookModuleSource(text, label, file.relativePath)
        if (
          inspection.commonJsReference ||
          inspection.dynamicImport ||
          inspection.staticImports.some((specifier) => specifier === "node:module" || specifier === "bun:module" || (!specifier.startsWith("node:") && !specifier.startsWith("bun:")))
        ) {
          error(label, `Hook module must be self-contained: ${file.relativePath}`)
        }
        if (!inspection.hasPluginExport) {
          error(label, `Hook module must export an OpenCode Plugin entry: ${file.relativePath}`)
        }
        continue
      }
      if (/https?:\/\//i.test(text) || /\b(?:fetch|WebSocket|XMLHttpRequest|EventSource)\s*\(/.test(text) || /navigator\.sendBeacon\s*\(/.test(text))
        error(label, `remote runtime dependency is forbidden: ${file.relativePath}`)
      if (
        text.startsWith("#!") ||
        extension === ".cjs" ||
...
async function validatePackageWorkspace(candidate, metadata) {
  ...
  if (!isObject(packageJson.scripts) || typeof packageJson.scripts.validate !== "string" || typeof packageJson.scripts.pack !== "string") {
...
function parsePetLibrary(value, label) {
  ...
  const pets = value.pets.map((item, index) => {
    ...
    exactKeys(item, ["alt", "description", "displayName", "id", "spritesheet", "spriteVersion"], ["alt", "description", "displayName", "id", "spritesheet", "spriteVersion"], itemLabel)
...
export function validatePetPackageLibrary(manifest, files, label = "Plugin") {
  ...
  for (const petEntry of library.pets) {
    ...
    const dimensions = extension === ".png" ? pngDimensions(asset.data, `${label} pet spritesheet ${petEntry.id}`) : webpDimensions(asset.data, `${label} pet spritesheet ${petEntry.id}`)
...
export function inspectShowcaseMedia(input, mime, label = "showcase media") {
  ...
  if (!Number.isSafeInteger(result.width) || !Number.isSafeInteger(result.height) || result.width < 1 || result.height < 1 || result.width > 8192 || result.height > 8192) {
...
export async function loadShowcaseAssets(metadata, packageDirectory, label = `${metadata.kind}/${metadata.id} showcase`) {
  ...
  const animation = metadata.showcase.animation ? await readShowcaseMedia(packageDirectory, metadata.showcase.animation, "animation", `${label} animation`) : undefined
  return { poster, ...(animation ? { animation } : {}) }
}
...
async function validateCompanionSourceDirectory(companion, label, workspaceRoot = root) {
  ...
  for (const target of companion.targets) {
    ...
    if (!isObject(sourcePackage.scripts) || typeof sourcePackage.scripts[script] !== "string" || sourcePackage.scripts[script].trim().length === 0) {
...
export async function discoverPackages(options = {}) {
  ...
  const selection = options.kind === undefined && options.id === undefined ? undefined : { kind: options.kind, id: options.id }
  if (selection && ((selection.kind !== "plugin" && selection.kind !== "skill") || typeof selection.id !== "string")) {
    error("package selection", "kind and id must select one Plugin or Skill")
  }
  const candidates = [...(await listCollection("plugin", workspaceRoot)), ...(await listCollection("skill", workspaceRoot))].sort((left, right) =>
  ...
  async function loadCandidate(candidate) {
    ...
    if (candidate.kind === "plugin") {
      ...
      assertPluginStatic(files, `${candidate.kind}/${candidate.id}`, manifest.hooks)
      ...
      if (manifest.hooks && !names.has(manifest.hooks)) error(`${candidate.kind}/${candidate.id}`, `missing hooks ${manifest.hooks}`)
      ...
      for (const companion of companions) {
        const sourcePackage = await validateCompanionSourceDirectory(companion, `${candidate.kind}/${candidate.id} companion ${companion.command}`, workspaceRoot)
  ...
  const packages = [...loaded.values()].sort((left, right) => `${left.kind}/${left.id}`.localeCompare(`${right.kind}/${right.id}`, "en"))
  composeOwnedSkillPackages(packages)
  return packages
}

export function composeOwnedSkillPackages(packages) {
  const standaloneSkills = new Map(packages.filter((pkg) => pkg.metadata.kind === "skill").map((pkg) => [pkg.metadata.id, pkg]))
...
function parseShowcaseMediaArtifact(value, metadata, role, label) {
  exactKeys(value, ["alt", "height", "mime", "sha256", "size", "url", "width"], ["alt", "height", "mime", "sha256", "size", "url", "width"], label)
  const mime = cleanString(value.mime, `${label} mime`, 80)
...
export function parseShowcaseEntry(value, label = "Showcase entry") {
  exactKeys(value, ["animation", "id", "kind", "poster", "schema", "version"], ["id", "kind", "poster", "schema", "version"], label)
...
export function parseRegistryEntry(value, label = "Registry entry") {
  ...
  const required = ["kind", "id", "name", "description", "version", "compatibility", "artifact", "yanked", ...(value.kind === "plugin" ? ["manifest"] : [])]
  ...
  const metadata = parseSourceMetadata(
    {
      ...
      yanked: value.yanked,
      ...(value.kind === "skill" && value.ownerPluginId !== undefined ? { ownerPluginId: value.ownerPluginId } : {}),
    },
    label,
  )
  const result = {
...
export function parseRegistry(value, label = "Registry") {
  ...
  const identities = packages.map((entry) => `${entry.kind}/${entry.id}`)
  if (new Set(identities).size !== identities.length) error(label, "contains more than one version for a package")
  const pluginsById = new Map(packages.filter((entry) => entry.kind === "plugin").map((entry) => [entry.id, entry]))
  const skillsById = new Map(packages.filter((entry) => entry.kind === "skill").map((entry) => [entry.id, entry]))
  for (const skill of packages.filter((entry) => entry.kind === "skill" && entry.ownerPluginId)) {
    ...
    const contribution =
      owner && (owner.manifest.schema === "convax.plugin/4" || owner.manifest.schema === "convax.plugin/5" || owner.manifest.schema === "convax.plugin/6")
  ...
  for (const plugin of packages.filter(
    (entry) => entry.kind === "plugin" && (entry.manifest.schema === "convax.plugin/4" || entry.manifest.schema === "convax.plugin/5" || entry.manifest.schema === "convax.plugin/6"),
  )) {
  ...
  const artifactUrls = packages.flatMap((entry) => [
    entry.artifact.url,
    ...(entry.kind === "plugin" && entry.companions ? entry.companions.flatMap((companion) => companion.targets.map((target) => target.artifact.url)) : []),
  ])
