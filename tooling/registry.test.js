...
describe("source packages", () => {
  test("validates the complete Plugin and Skill catalog", async () => {
    ...
    expect(
      packages.map((pkg) => `${pkg.metadata.kind}/${pkg.metadata.id}`),
    ).toEqual([
      "plugin/chatcut",
    ...
    const ffmpegSkill = packages.find(
      (pkg) =>
        pkg.metadata.kind === "skill" && pkg.metadata.id === "ffmpeg-canvas",
    );
    const hello = packages.find((pkg) => pkg.metadata.id === "hello-convax");
    const jianying = packages.find(
      (pkg) =>
        pkg.metadata.id === "jianying-editor" && pkg.metadata.kind === "plugin",
    );
    const codex = packages.find((pkg) => pkg.metadata.id === "codex-service");
    const multiAngle = packages.find(
      (pkg) => pkg.metadata.id === "multi-angle",
    );
    const panorama = packages.find(
      (pkg) => pkg.metadata.id === "panorama-viewer",
    );
    const director = packages.find(
      (pkg) => pkg.metadata.id === "storyai-3d-director-desk",
    );
    const Example Generation = packages.find(
      (pkg) => pkg.metadata.id === "example-generation",
    );
    ...
    expect(jianying.manifest).toEqual(
    ...
    expect(codex.manifest.contributes.service.actions).toEqual([
      "authorize",
      "reauthorize",
    ]);
    ...
    expect(multiAngle.manifest).toEqual(
      expect.objectContaining({
        capabilities: [
    ...
    expect(panorama.manifest).toEqual(
      expect.objectContaining({
        capabilities: [
          "canvas.connectedImages.read",
          "canvas.image.write",
    ...
    expect(director.manifest).toEqual(
      expect.objectContaining({
    ...
    expect(Example Generation.manifest).toEqual(
      expect.objectContaining({
        capabilities: [],
    ...
    expect(
      Example Generation.manifest.contributes.generation.models.map(
        (model) => model.tool,
      ),
    ).toEqual(generationTools.map((tool) => tool.id));
    expect(Example Generation.manifest.contributes.generation.models[0].name).toBe(
      "Example Image",
    );
    expect(
      Example Generation.manifest.contributes.generation.models.find((model) =>
        model.tool.startsWith("video."),
      )?.name,
    ...
    expect(ffmpeg.manifest).toEqual(
      expect.objectContaining({
    ...
    expect(
      ffmpeg.manifest.contributes.generation.tools.map((tool) => [
        tool.id,
        tool.output,
      ]),
    ).toEqual([
    ...
    for (const tool of ffmpeg.manifest.contributes.generation.tools.slice(
      0,
      3,
    )) {
      expect(tool.acceptedInputs).toEqual([
    ...
    expect(
      ffmpeg.manifest.contributes.canvas.selectionActions.map((action) => [
        action.id,
        action.editor,
      ...
      [
        ...
        "confirmation",
        ["video.without-audio", "audio.extract"],
      ],
    ...
    for (const source of ffmpegSkill.files) {
      const embedded = ffmpeg.files.find(
        (file) =>
          file.relativePath === `skills/ffmpeg-canvas/${source.relativePath}`,
      );
  ...
  test("creates byte-identical ZIPs with required root entries", async () => {
    const packages = await discoverPackages();
    const first = await packPackages(
      packages,
      path.join(await temporaryDirectory(), "first"),
    );
    const second = await packPackages(
      packages,
      path.join(await temporaryDirectory(), "second"),
    );
    expect(first.map((item) => sha256(item.zip))).toEqual(
      second.map((item) => sha256(item.zip)),
    );
    ...
    expect(
      readStoredZip(hello.zip).map((entry) => entry.relativePath),
    ).toContain("manifest.json");
    expect(readStoredZip(codex.zip).map((entry) => entry.relativePath)).toEqual(
    ...
    expect(
      readStoredZip(multiAngle.zip).map((entry) => entry.relativePath),
    ).toEqual([
      "LICENSE",
      "assets/app.js",
    ...
    expect(
      readStoredZip(Example Generation.zip).map((entry) => entry.relativePath),
    ).toEqual(["LICENSE", "manifest.json"]);
    ...
    expect(await fs.readFile(Example Generation.companionAssets[0].path)).toEqual(
      Example Generation.companionAssets[0].data,
    );
    expect(
      readStoredZip(skill.zip).map((entry) => entry.relativePath),
    ).toContain("SKILL.md");
    expect(
      readStoredZip(ffmpeg.zip).map((entry) => entry.relativePath),
    ).toEqual([
      "FFMPEG-CREDITS",
      "FFMPEG-LICENSE",
    ...
    expect(ffmpegLicense?.data).toEqual(
      await fs.readFile(
        path.join(root, "packages", "tools", "ffmpeg-mcp", "FFMPEG-LICENSE"),
      ),
    );
  });
  ...
  test("rechecks combined owned-Skill size and canonical path collisions", async () => {
    ...
    const uncomposedPackages = () =>
      packages.map((pkg) => {
        if (pkg.metadata.kind !== "plugin") return pkg;
        const ownedSkillPrefixes = (pkg.manifest.contributes.skills ?? []).map(
        ...
        return {
          ...
          files: pkg.files.filter(
            (file) =>
              !ownedSkillPrefixes.some((prefix) =>
    ...
    const oversizedPackages = uncomposedPackages().map((pkg) =>
      pkg.metadata.kind === "plugin" && pkg.metadata.id === ffmpeg.metadata.id
        ...
        : pkg.metadata.kind === "skill" &&
            pkg.metadata.id === ffmpegSkill.metadata.id
          ? {
              ...pkg,
              files: [
                ...pkg.files,
    ...
    expect(() => composeOwnedSkillPackages(oversizedPackages)).toThrow(
      "package exceeds 10 MiB",
    );
    ...
    expect(() => composeOwnedSkillPackages(collisionPackages)).toThrow(
      "owned Skill path collides",
    );
    ...
    expect(() => composeOwnedSkillPackages(entryBoundPackages)).toThrow(
      "package exceeds the 2000 entry limit",
    );
  });
  ...
  test("builds the strict client Registry with only the latest stable version", async () => {
    ...
    const hello = packedResults.find(
      (item) => item.pkg.metadata.id === "hello-convax",
    );
    ...
    await fs.writeFile(
      path.join(newerDirectory, "registry-entry.json"),
      `${JSON.stringify(newer, null, 2)}\n`,
    );
    const registry = await buildIndex({
      entriesDirectory: packed,
      outputFile: output,
      revision: "a".repeat(40),
      sequence: 7,
    });
    expect(
      parseRegistry(JSON.parse(await fs.readFile(output, "utf8"))),
    ).toEqual(registry);
    ...
    const helloEntry = registry.packages.find(
      (item) => item.id === "hello-convax",
    );
    const Example GenerationEntry = registry.packages.find(
      (item) => item.id === "example-generation",
    );
    const firstSkill = registry.packages.find((item) => item.kind === "skill");
    const ffmpegSkillEntry = registry.packages.find(
      (item) => item.kind === "skill" && item.id === "ffmpeg-canvas",
    );
    ...
    delete missingOwner.packages.find(
      (item) => item.kind === "skill" && item.id === "ffmpeg-canvas",
    ).ownerPluginId;
    ...
    missingSkill.packages = missingSkill.packages.filter(
      (item) => item.kind !== "skill" || item.id !== "ffmpeg-canvas",
    );
  ...
  test("requires a Release ZIP whose size matches its Registry entry", async () => {
    ...
    const release = {
      assets: [
        {
          name: "registry-entry.json",
          url: "https://api.github.test/assets/entry",
        },
    ...
    const fetchImpl = async (url) => {
      ...
      if (url === "https://api.github.test/assets/entry")
        return new Response(JSON.stringify(packed.entry));
      throw new Error(`Unexpected URL ${url}`);
    };
    const output = path.join(directory, "release-entries");
    expect(
      await fetchReleaseEntries({
    ...
    expect(
      JSON.parse(
        await fs.readFile(
          path.join(output, packed.tag, "registry-entry.json"),
          "utf8",
        ),
      ),
    ).toEqual(packed.entry);
  ...
  test("downloads and verifies every declared companion executable", async () => {
    ...
    ).rejects.toThrow("size does not match Registry entry");
    ...
    ).rejects.toThrow("digest does not match Registry entry");
  ...
  test("audits source versions against their immutable Release entries", async () => {
    ...
    await expect(
      checkReleaseCoverage({
        ...
        packages: [changedHello],
      }),
    ).rejects.toThrow(
      "Release artifact does not match the current source package",
    );
...
describe("hello-convax runtime", () => {
  test("accepts one scoped port and renders host.context.get", async () => {
    ...
    const code = await fs.readFile(
      path.join(root, "packages/plugins/hello-convax/package/assets/app.js"),
      "utf8",
    );
    ...
    expect(elements.status.textContent).toBe(
      "Connected through convax.plugin-host/1.",
    );
