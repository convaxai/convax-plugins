...
describe("convax.package/2 and convax.plugin/8 publication contract", () => {
  ...
  test("keeps publication policy outside portable package metadata", () => {
    ...
    ).toThrow("unsupported field requests")
    expect(() =>
      parseHostCapabilityPolicy({
        schema: "convax.host-capability-policy/3",
        blockers: [],
    ...
    ).toThrow("contract digest does not match the current Catalog")
  })
  ...
  test("rejects human-review blocker codes", () => {
    expect(() =>
      blockedPublication([
        {
          code: "host-capability-review-required",
          note: "Wait for a reviewer.",
        },
      ]),
    ).toThrow("unsupported blocker code")
  })
      ...
      manifest({
        contributes: {
          canvas: {
            ...
            toolbar: [{ id: "refresh", command: "context.refresh", order: 10 }],
            menus: [
              {
                id: "refresh-overflow",
  ...
  test("keeps Skill Host APIs within top-level declaration and tools within contributions", () => {
    ...
    outside.contributes.skills[0].uses.optionalHostApis = [
